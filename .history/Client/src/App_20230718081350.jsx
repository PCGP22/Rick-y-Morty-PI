...
function App() {
  ...
  async function onSearchHome(id){
    try{
      const {data} = await axios(`http://localhost:5173/rickandmorty/character/${id}`)
      if (data.name){
  ...
  return (
    <>
      <Routes>
          ...
          <Route path='/rickandmorty/character/:id' element={<Details onSearch={onSearch} logout={logout}/>}/>
